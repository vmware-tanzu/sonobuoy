...
push_manifest:
	$(DOCKER_BUILD) 'manifest-tool  --docker-cfg $(HOME)/.docker/config.json push from-args --platforms $(PLATFORMS) --template $(REGISTRY)/$(TARGET)-ARCH:$(VERSION) --target  $(REGISTRY)/$(TARGET):$(VERSION)'
