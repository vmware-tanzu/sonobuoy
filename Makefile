...
push_manifest:
	$(DOCKER_BUILD) 'manifest-tool -username oauth2accesstoken --password "`/root/google-cloud-sdk/bin/gcloud auth print-access-token`" push from-args --platforms $(PLATFORMS) --template $(REGISTRY)/$(TARGET)-ARCH:$(VERSION) --target  $(REGISTRY)/$(TARGET):$(VERSION)'
