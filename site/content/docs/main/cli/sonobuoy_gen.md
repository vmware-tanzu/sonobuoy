...
```
  ...
  -p, --plugin pluginList                        Which plugins to run. Can either point to a URL, local file/directory, or be one of the known plugins (e2e or systemd-logs). Can be specified multiple times to run multiple plugins.
      ...
      --sonobuoy-image string                    Container image override for the sonobuoy worker and aggregator. (default "sonobuoy/sonobuoy:v0.56.6-1-g7da578e1-dirty")
...
* [sonobuoy gen cli](sonobuoy_gen_cli.md)	 - Generates markdown docs for the CLI
* [sonobuoy gen config](sonobuoy_gen_config.md)	 - Generates a Sonobuoy config for input to sonobuoy gen or run.
